...
if './UKGE/src' not in sys.path:
    sys.path.append('./UKGE/src')

if './UKGE/' not in sys.path:
    sys.path.append('./UKGE/')
...
data_dir = join('./UKGE/data', args.data)
...
model_dir = './UKGE/trained_models/3D_CHESS/rect_1030/'
data_filename = 'data.bin'
model_filename = 'model.bin'
start = time.time()
validator.build_by_file('./UKGE/data/3D_CHESS/query.tsv', model_dir, model_filename, data_filename)
end = time.time()
print(end-start)
...
with open("./data/KG raw data/prob_kg.csv", "r") as kg_file:
    next(kg_file)
...
for i in range(len(instrument_names)):
    with open("./data/KG processed data/en2id.txt", "r") as entity_file:
