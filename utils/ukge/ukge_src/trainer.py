...
class Trainer(object):
    ...
    def train(self, epochs=20, save_every_epoch=10, lr=0.001, data_dir=""):
        ...
        for epoch in range(1, epochs + 1):
            ...
            if epoch % save_every_epoch == 0:
                ...
                print('VALIDATE:')
                val_mse, val_mse_neg, val_mae, val_mae_neg  = self.get_val_loss(epoch, sess)  # loss for testing triples and negative samples
                val_losses.append([epoch, val_mse, val_mse_neg, val_mae, val_mae_neg])

                # save and print metrics
                print('TRAIN: Save Loss')
                self.save_loss(train_losses, self.train_loss_path, columns=['epoch', 'training_loss'])
                #self.save_loss(val_losses, self.val_loss_path, columns=['val_epoch', 'mse', 'mse_neg', 'mae', 'mae_neg'])
                print('VALIDATE:Save Loss')
                self.save_loss(val_losses, self.val_loss_path, columns=['val_epoch', 'mse', 'mse_neg', 'mae', 'mae_neg'])
    ...
    def get_val_loss(self, epoch, sess):
        ...
        # print ("hr_map200",tf.shape(hr_map200))
        # mean_ndcg, mean_exp_ndcg = self.validator.mean_ndcg(hr_map200)
        # print ("mean_ndcg",mean_ndcg)
        # print ("mean_exp_ndcg",mean_exp_ndcg)
        
        print ("score of val")
        # print score of val
        self.validator.get_val()
        ...
        return mse, mse_neg, mae, mae_neg 
