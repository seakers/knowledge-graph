...
#print ("number of query", len(this_data.val_triples))
...
validator.build_by_file(data_dir+'/query.tsv', model_dir, model_filename, data_filename)
end = time.time()
#print(end-start)
scores = validator.get_val()

threshold = 0.75
valid_relations = scores > threshold
count = 0
for relation in valid_relations:
    if relation:
        count+=1

print("Out of "+str(len(valid_relations))+" possible relations to add, "+str(count)+" have scores above the threshold of "+str(threshold)+".")

head_ids = []
tail_ids = []
relation_ids = []
query_filename = data_dir+'/query.tsv'
with open(query_filename, "r") as query_file:
    tsv_file = csv.reader(query_file, delimiter="\t")
    # printing data line by line
    idx = 0
    for line in tsv_file:
        if valid_relations[idx] == 1:
            head_ids.append(line[0])
            tail_ids.append(line[2])
            relation_ids.append((line[1]))
        idx+=1

with open("../valid_ukge_relations.csv","w") as csvfile:
    fieldnames = ['head_id', 'relation_id', 'tail_id']
